...
int floppy_rw( unsigned flp, unsigned track, BYTE cmd )
{
	...
	for( i = 0; i < 10; ++i )
	{
		open_dma_channel( FLOPPY_DMA_CHANNEL, lpFlpDmaBuf, FLOPPY_DMA_LEN-1, DMA_MODE_SINGLE|((cmd==CMD_READ_DATA)?(DMA_MODE_READ):(DMA_MODE_WRITE)) );
...
int flp_dev_read( io_dev_t *dev, BYTE *buf, UINT64 offset, UINT64 count )
{
	...
	memcpy( (void*)buf, (PVOID)((BYTE*)lpFlpDmaBuf)+((unsigned)offset)%track_size, t );
	count -= t;
...
int flp_dev_write( io_dev_t *dev, const BYTE *buf, UINT64 offset, UINT64 count )
{
	...
	for( track=((unsigned)offset)/track_size; ((unsigned)count); ++track )
	{
		...
		if( r )
			return r;
		t = min( ((unsigned)count), track_size-(((unsigned)offset)%track_size) );
		memcpy( (PVOID)((BYTE*)lpFlpDmaBuf)+((unsigned)offset)%track_size, (void*)buf, t);
		*((unsigned*)&count) -= t;
	}
