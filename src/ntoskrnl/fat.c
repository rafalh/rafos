...
int fat_readfile(struct volume_t *vol, char *buf, unsigned c, unsigned cluster)
{
    ...
    if(cluster == 0)
    {
        ...
        if(vol->dev->read(vol->dev, (BYTE*)buf, offset, ((fat_bs_t*)vol->param)->root_entry_count * sizeof(fat_dir_entry_t)))
            return -1;
        return 0;
    }
    ...
    do
    {
        ...
        if(vol->dev->read(vol->dev, (BYTE*)buf, offset, c2))
            return -1;
...
int fat_init(struct volume_t *vol)
{
    ...
    {
        printf("fat_init: not FAT12 volume (clusters %u sig %x)\n", total_clusters, ((fat_extbs_t*)fat_data->bs.extended_section)->signature);
        goto e_free_fat_data;
    }
...
int fat_getinfo(struct volume_t *vol, UINT32 *serial_nr, char *label, unsigned labelsize, unsigned *max_comp_len)
{
    ...
    if(label)
    {
        int i;
        i = min(labelsize, 11);
        strncpy(label, (char*)(((fat_extbs_t*)((fat_data_t*)vol->param)->bs.extended_section)->volume_label), i);
        for(; i >= 0 && label[i] == ' '; --i);
        label[i + 1] = '\0';
    }
    ...
    for(; path[0];)
    {
        ...
        for(i = 0; i < ((*result)->c); ++i)
        {
            ...
            if(*((BYTE*)entry) && *((BYTE*)entry) != 0xE5 && (entry->attributes & FAT_DIRECTORY) && j == len && !strnicmp(path, entry->name, len))
            {
                ...
                fat_readfile(vol, (char*)(*result)->data, 0xffffffff, entry->first_cluster_lo);
